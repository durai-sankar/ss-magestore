...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    public function importFromTextArray(array $row)
    {
        ...
        if (!$this->validateAddress($row, 'billing')) {
			...
			$billingAddress->setBlockno($row['billing_blockno']);
			$billingAddress->setUnitno($row['billing_unitno']);
        ...
        if (!$this->validateAddress($row, 'shipping')) {
			...
			$shippingAddress->setBlockno($row['shipping_blockno']);
			$shippingAddress->setUnitno($row['shipping_unitno']);
    ...
    {
        $fields = array('city', 'country', 'postcode', 'telephone', 'street1', 'blockno');
        $usca   = array('US', 'CA');
        $prefix = $type ? $type . '_' : '';
