...
class VladimirPopov_WebForms_Adminhtml_WebFormsController extends Mage_Adminhtml_Controller_Action
{
    protected function _initAction()
    {
        $this->loadLayout()->_setActiveMenu('webforms/webforms');
		
        if ((float)substr(Mage::getVersion() , 0, 3) > 1.3) $this->_title($this->__('Web-forms'))->_title($this->__('Manage Forms'));
    ...
    {
        $this->loadLayout();
        $this->getResponse()->setBody($this->getLayout()->createBlock('webforms/adminhtml_webforms_grid')->toHtml());
    }
    ...
    public function editAction()
    {
        ...
        if ((float)substr(Mage::getVersion() , 0, 3) > 1.3 || Mage::helper('webforms')->getMageEdition() == 'EE') $this->_title($this->__('Web-forms'))->_title($this->__('Edit Form'));
        
        $webformsId = $this->getRequest()->getParam('id');

        $forms = Mage::getModel('webforms/webforms')->getCollection();
        if (count($forms) >= 300 && empty($webformsId)) {
            $this->_redirect('*/*/');
        }

        $webformsModel = Mage::getModel('webforms/webforms')->setStoreId($this->getRequest()->getParam('store'))->load($webformsId);
        ...
        if ($webformsModel->getId() || $webformsId == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('webforms/adminhtml_webforms_edit'))->_addLeft($this->getLayout()->createBlock('webforms/adminhtml_webforms_edit_tabs'));
            $this->renderLayout();
        } else {
    ...
    public function saveAction()
    {
        if ($this->getRequest()->getPost()) {
            ...
            try {
                ...
                if ($store) $form->saveStoreData($store, $postData);
                else $form->setData($postData)->setId($id)->setUpdateTime(Mage::getSingleton('core/date')->gmtDate())->save();
                
                if ($this->getRequest()->getParam('id') <= 0) $form->setCreatedTime(Mage::getSingleton('core/date')->gmtDate())->save();
